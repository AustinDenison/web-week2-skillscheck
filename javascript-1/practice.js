...
const middleNums = numbers.slice(1, 4) //Code Here
...
const me = {
	...
	age: 20,
	greeter(){
		const greet = `Hello! my name is ${this.firstName} and i live in ${state}`
	}
} //Code Here
...
const doubled = myNumbers.map(n => n*2) // Code Here

//////////////////PROBLEM 15////////////////////

// Now lets use .filter to only get the nubmer(s) greater than 100. Call the new array 'filtered'

const filtered = myNumbers.filter(n => { if(n > 100) return n} ) // Code Here

//////////////////PROBLEM 16////////////////////

// Next up, reduce to get the total of the array. call the new array 'total'

const total = myNumbers.reduce((acc, cur) => acc += cur) // Code Here
...
myNumbers.forEach((n, i) => {myNumbersIndex.push(i)}) // Code Here
...
const forTheLoveOfGeorge = notGeorge.map(n => n = 'George') // Code Here
...
const enemies = people.filter(n => { if(n.friend == false) return n}) // Code Here
...
const totallyAwesome = people.awesomeLevel.reduce((acc, cur) =>  acc += cur) // Code Here
